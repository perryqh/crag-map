...
- **Real Room migrations for field-survey data.** `PinOverrideDatabase` still uses `fallbackToDestructiveMigration()`, which is the right call for the bundled read-only `AppDatabase` (a disposable snapshot recreated on every data update) but the wrong one for a user's own captured pins/photos — worth swapping to real migrations before real field data is actually at stake.
